...
# for file in os.listdir("./HIST_PAINEL_COVIDBR_07nov2022"):
#        df = pd.read_csv(f"./HIST_PAINEL_COVIDBR_07nov2022/{file}", sep=';')
#       df_states = pd.concat([df_states, df[(~df["estado"].isna()) & (df["codmun"].isna())]])
#       df_brasil = pd.concat([df_brasil, df[df["regiao"] == "Brasil"]])
        ...
        dbc.Row([
            dbc.Col([
                html.Div([
                    ...
                    html.H2(children="Evolução COVID-19 no Brasil"),
